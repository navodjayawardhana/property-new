...
export default function HomePageClient() {
  ...
  useEffect(() => {
    Promise.all([
      propertiesApi.list({ listing_type: "buy", featured: true, per_page: 4 }),
      propertiesApi.list({ listing_type: "rent", per_page: 4 }),
      agentsApi.list({ page: 1 }),
      newsApi.list({ per_page: 4 }),
    ])
      .then(([buyRes, rentRes, agentsRes, newsRes]) => {
        setBuyProps(buyRes.data.slice(0, 4));
        setRentProps(rentRes.data.slice(0, 4));
      ...
      <section className="max-w-7xl mx-auto px-4 pt-6 pb-10 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {loading
            ...
            : buyProps.length === 0
              ? <p className="col-span-4 text-gray-400 text-center py-10">No properties listed yet.</p>
      ...
      <section className="max-w-7xl mx-auto px-4 pb-10 w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
          {loading
            ? Array.from({ length: 4 }).map((_, i) => <PropertyCardSkeleton key={i} />)
            : rentProps.length === 0
              ? <p className="col-span-4 text-gray-400 text-center py-10">No rentals listed yet.</p>
