...
export default function SignInPage() {
  ...
  const { login, verifyOtp, otpPending } = useAuth();
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result === "done") router.push("/");
      if (result === "verify") setError("Your email is not verified. A new verification code has been sent to your email — please check your inbox and verify your account first.");
    } catch (err: unknown) {
        ...
        <div className="w-full max-w-md">
            ...
            {ROLES.map((r) => (
              <button
                ...
                className={`flex-1 flex flex-col items-center gap-0.5 py-2.5 rounded-lg text-xs font-semibold transition-all ${
                  role === r.key
                    ? "bg-white text-[#121e80] shadow-sm"
                    : "text-gray-500 hover:text-gray-700"
                }`}
              >
                {r.icon}
                <span>{r.label}</span>
                <span className={`text-[10px] font-normal ${role === r.key ? "text-gray-400" : "text-gray-400"}`}>{r.desc}</span>
              </button>
            ))}
          </div>

          {/* OTP hint for non-admin */}
          {role !== "admin" && (
          ...
          {role !== "admin" && (
            <>
              ...
              <a
                href={`http://localhost:8000/auth/google/redirect?role=${role}`}
