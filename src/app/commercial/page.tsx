      ...
      <div className="bg-[#16a34a] py-12 px-4">
        <div className="max-w-7xl mx-auto">
          ...
          <p className="text-green-200 text-sm">
            Commercial and mixed-use properties across Sri Lanka.
          </p>
        </div>
      </div>
        ...
        {activeFilters.length > 0 && (
          <div className="flex flex-wrap gap-2 mt-3">
            {activeFilters.map(({ key, label }) => (
              <span
                key={key}
                className="flex items-center gap-1 bg-slate-50 border border-slate-300 text-slate-700 text-xs font-semibold px-2.5 py-1 rounded-full"
              >
                {label}
                <button
