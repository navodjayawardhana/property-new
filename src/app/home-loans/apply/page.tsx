...
import {
  ...
  MapPin, CheckCircle, ChevronLeft, Building2, CreditCard,
...
function ApplyForm() {
  ...
  const termStr   = params.get("term")    ?? "";
  const monthly   = parseInt(params.get("monthly") ?? "0");

  const [form, setForm] = useState<FormData>({
    name: "", nic_number: "", email: "", phone: "",
    employment_type: "full_time",
    annual_income: "", deposit_amount: "",
    loan_amount: amountStr, loan_term: termStr,
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-10 text-center max-w-lg mx-auto">
        ...
        {bankName && (
          <p className="text-sm text-gray-500 mb-2">
            Your application for <span className="font-bold text-gray-900">{bankName}</span> has been received.
          </p>
        )}
        <p className="text-gray-400 text-sm mb-2">
          Thank you, <span className="font-semibold text-gray-700">{form.name}</span>! We'll review your details and get back to you at{" "}
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      {(bankName || monthly > 0) && (
        <div className="bg-[#16a34a] rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center gap-4">
          <div className="flex items-center gap-3 flex-1 min-w-0">
            <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center shrink-0">
              <Building2 size={18} className="text-white" />
            </div>
            <div className="min-w-0">
              <p className="text-green-200 text-xs font-semibold uppercase tracking-wide">Applying for</p>
              <p className="text-white font-black text-lg truncate">{bankName || "Home Loan"}</p>
              {rateStr && <p className="text-green-300 text-xs">{parseFloat(rateStr).toFixed(2)}% p.a.</p>}
            </div>
          </div>
          <div className="flex gap-5 shrink-0">
            {monthly > 0 && (
              <div className="text-center">
                <p className="text-green-300 text-xs">Est. Monthly</p>
                <p className="text-white font-black">{fmt(monthly)}</p>
              </div>
            )}
            {amountStr && (
              <div className="text-center">
                <p className="text-green-300 text-xs">Loan Amount</p>
                <p className="text-white font-black">{fmt(parseInt(amountStr))}</p>
              </div>
            )}
            {termStr && (
              <div className="text-center">
                <p className="text-green-300 text-xs">Term</p>
                <p className="text-white font-black">{termStr} yrs</p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
        <Section icon={FileText} title="Additional Notes">
          <textarea value={form.message} onChange={(e) => set("message", e.target.value)}
            rows={3} placeholder="Any additional information you'd like to share with the bank..."
      ...
      <p className="text-center text-xs text-gray-400">
        Your information is kept confidential and used only to process your loan application.
      </p>
    </form>
  );
}
